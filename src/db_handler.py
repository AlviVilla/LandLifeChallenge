...
class DB_Handler:
    ...
    def __init__(self, conf):
        """Constructor to injest the data into de database
        Actions:
            DB creation
            Tables creation
            Retrieval of CSV data
            insertion of CSV data into tables
        """
        self.create_DB(conf)
        self.create_tables(conf)
        tree_data = self.data_retrieval(conf["data"]["field_data"])
        specs_data = self.data_retrieval(conf["data"]["species"])
        self.insert_in_field_data(conf, tree_data)
